...
docker run --rm \
  ...
  ceyoniq.azurecr.io/release/nscale/pipeliner:ubi.10.0.1400
```
...
Diese Datei können Sie dem Image `ceyoniq.azurecr.io/release/nscale/pipeliner:ubi.10.0.1400

Kopieren Sie die Datei `doc_mime_suff.tsv` lokal auf Ihr System:  

```bash
# Erzeugen eines temporären Containers
$ docker create ceyoniq.azurecr.io/release/nscale/pipeliner:ubi.10.0.1400
a0123456789 
...
```bash
docker run -it ... -v ${PWD}/doc_mime_suff.tsv:/opt/ceyoniq/nscale-pipeliner/workdir/config/common/doc_mime_suff.tsv ceyoniq.azurecr.io/release/nscale/pipeliner:ubi.10.0.1400
```
